...
def buscar_configuracoes():
    try:
        ...
        if response.status_code == 200 and response.json():
            ...
            return {"nome_remetente": "Liraprint", "endereco_remetente": "R. Dr. Roberto Fernandes, 81", "bairro_remetente": "Jardim Palmira", "cidade_remetente": "Guarulhos", "estado_remetente": "SP", "cep_remetente": "07076-070"}
    except:
        return {"nome_remetente": "Liraprint", "endereco_remetente": "R. Dr. Roberto Fernandes, 81", "bairro_remetente": "Jardim Palmira", "cidade_remetente": "Guarulhos", "estado_remetente": "SP", "cep_remetente": "07076-070"}
...
def listar_servicos():
    ...
    .status-pendente {{ color: #e67e22; font-weight: bold; }} .status-producao {{ color: #3498db; font-weight: bold; }} .status-concluido {{ color: #27ae60; font-weight: bold; }} .status-entregue {{ color: #2c3e50; font-weight: bold; }}
    .back-link {{ display: inline-block; margin: 20px 30px; color: #3498db; text-decoration: none; font-weight: 500; }}
    .footer {{ text-align: center; padding: 20px; background: #ecf0f1; color: #7f8c8d; font-size: 13px; border-top: 1px solid #bdc3c7; }}
    .tab-content {{ display: none; }} .tab-content.active {{ display: table-row-group; }}
    .search-box {{ text-align: center; padding: 20px; }} .search-box input {{ padding: 10px; width: 300px; border: 1px solid #ddd; border-radius: 8px; }}
    </style>
    </head>
    <body>
    <div class="container">
...
def adicionar_servico():
    ...
    <div class="grid-2"><div><label>Quantidade / Lote</label><input type="number" name="quantidade" step="1"></div><div><label>Nº de Cores</label><input type="number" name="numero_cores" step="1"></div></div>
...
def editar_servico(id):
    ...
    <div class="grid-2"><div><label>Quantidade / Lote</label><input type="number" name="quantidade" value="{servico.get('quantidade', '')}" step="1"></div><div><label>Nº de Cores</label><input type="number" name="numero_cores" value="{servico.get('numero_cores', '')}" step="1"></div></div>
...
def pdf_os(id):
    ...
    empresa_nome = servico['empresas']['nome_empresa'] if servico.get('empresas') else "Sem cliente"
    logo_url = "https://i.postimg.cc/RVqcJzzQ/logo.png"
    html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>OS {servico['codigo_servico']}</title><style>body {{ font-family: Arial, sans-serif; padding: 40px; background: white; }}.header {{ text-align: center; margin-bottom: 20px; border-bottom: 2px solid #2c3e50; padding-bottom: 15px; }}.header img {{ max-width: 200px; margin-bottom: 10px; }}.header h1 {{ margin: 0; color: #2c3e50; font-size: 24px; text-transform: uppercase; letter-spacing: 1px; }}table {{ width: 100%; border-collapse: collapse; margin: 20px 0; }}th, td {{ border: 1px solid #ccc; padding: 8px; text-align: left; }}th {{ background-color: #ecf0f1; color: #2c3e50; }}.total-box {{ text-align: right; font-size: 18px; margin-top: 20px; }}.footer {{ margin-top: 40px; text-align: center; padding: 20px; border-top: 1px solid #ddd; font-size: 12px; color: #7f8c8d; }}</style></head><body><div class="header"><img src="{logo_url}" alt="Logo da Empresa"><h1>ORDEM DE SERVIÇO</h1><p><strong>Código:</strong> {servico['codigo_servico']}</p></div><table><tr><th>Cliente</th><td>{empresa_nome}</td></tr><tr><th>Título</th><td>{servico['titulo']}</td></tr><tr><th>Status</th><td>{servico['status']}</td></tr><tr><th>Quantidade</th><td>{servico.get('quantidade', '—')}</td></tr><tr><th>Dimensão</th><td>{servico.get('dimensao', '—')}</td></tr><tr><th>Nº de Cores</th><td>{servico.get('numero_cores', '—')}</td></tr><tr><th>Aplicação</th><td>{servico.get('aplicacao', '—')}</td></tr><tr><th>Data Abertura</th><td>{format_data(servico.get('data_abertura'))}</td></tr><tr><th>Previsão Entrega</th><td>{format_data(servico.get('previsao_entrega'))}</td></tr><tr><th>Valor Cobrado</th><td>R$ {valor_cobrado:.2f}</td></tr><tr><th>Custo Materiais</th><td>R$ {custo_materiais:.2f}</td></tr><tr><th>Lucro</th><td>R$ {lucro:.2f}</td></tr><tr><th>Observações</th><td>{servico.get('observacoes', '—')}</td></tr></table><h3>Materiais Utilizados</h3><table><thead><tr><th>Material</th><th>Qtd</th><th>Valor Unit.</th><th>Valor Total</th></tr></thead><tbody>{''.join(f"<tr><td>{m['materiais']['denominacao']}</td><td>{m['quantidade_usada']}</td><td>R$ {m['valor_unitario']:.2f}</td><td>R$ {m['valor_total']:.2f}</td></tr>" for m in servico.get('materiais_usados', []) if m.get('materiais'))}</tbody></table><div class="total-box"><p><strong>Lucro Final:</strong> R$ {lucro:.2f}</p></div><div class="footer">Sistema de Gestão para Gráfica Rápida | © 2025</div></body></html>'''
...
def salvar_configuracoes_view():
    ...
    if salvar_configuracoes(config): flash("✅ Configurações salvas com sucesso!")
    else: flash("❌ Erro ao salvar configurações.")
...
def pdf_orcamento(id):
    ...
    html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Orçamento {orc['codigo_servico']}</title><style>body {{ font-family: Arial, sans-serif; padding: 40px; background: white; }}.header {{ text-align: center; margin-bottom: 20px; border-bottom: 2px solid #2c3e50; padding-bottom: 15px; }}.header h1 {{ margin: 0; color: #2c3e50; }}table {{ width: 100%; border-collapse: collapse; margin: 20px 0; }}th, td {{ border: 1px solid #ccc; padding: 10px; text-align: left; }}th {{ background-color: #ecf0f1; }}.total-box {{ text-align: right; font-size: 18px; margin-top: 20px; }}.footer {{ margin-top: 40px; text-align: center; padding: 20px; border-top: 1px solid #ddd; font-size: 12px; color: #7f8c8d; }}.data-entrega {{ background: #e8f5e9; padding: 10px; border-radius: 8px; margin: 15px 0; border-left: 4px solid #27ae60; }}</style></head><body><div class="header"><h1>ORÇAMENTO</h1><p><strong>Código:</strong> {orc['codigo_servico']}</p></div><div class="data-entrega"><strong>📅 Data Prevista de Entrega:</strong> {orc.get('previsao_entrega', '—').split('T')[0] if orc.get('previsao_entrega') else '—'} (calculada pulando fds/feriados)</div><table><tr><th>Cliente</th><td>{orc['empresas']['nome_empresa'] if orc.get('empresas') else '—'}</td></tr><tr><th>Abertura</th><td>{format_data(orc.get('data_abertura'))}</td></tr><tr><th>Status</th><td>{orc['status']}</td></tr></table><h3>Itens</h3><table><thead><tr><th>Descrição</th><th>Qtd</th><th>Dimensão</th><th>Cores</th><th>Valor Unit.</th><th>Total</th></tr></thead><tbody>{''.join(f"<tr><td>{i['titulo']}</td><td>{i['quantidade']}</td><td>{i.get('dimensao','')}</td><td>{i.get('numero_cores','')}</td><td>R$ {i['valor_unitario']:.2f}</td><td>R$ {i['valor_total']:.2f}</td></tr>" for i in itens)}</tbody></table><div class="total-box"><p><strong>VALOR TOTAL:</strong> R$ {float(orc.get('valor_cobrado', 0) or 0):.2f}</p></div><div class="footer">Sistema de Gestão para Gráfica Rápida | © 2025</div></body></html>'''
    pdf = pdfkit.from_string(html, False)
...
def marcar_entregue_view(id):
    if 'usuario' not in session: return redirect(url_for('login'))
    if marcar_entregue(id): flash("✅ Entrega registrada com sucesso!")
